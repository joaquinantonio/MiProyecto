...
public class DoctorChooseSlots extends AppCompatActivity {
    ...
    private DoctorViewSlotsAdapter adapter;
...
//        email = email.replace(".",",");
        ...
        reference_appt = FirebaseDatabase.getInstance("https://mi-proyecto-8c7aa-default-rtdb.asia-southeast1.firebasedatabase.app/").getReference("Doctors_Appointments");
        ...
        reference_token = FirebaseDatabase.getInstance("https://mi-proyecto-8c7aa-default-rtdb.asia-southeast1.firebasedatabase.app/").getReference("Tokens");
        ...
        recyclerView.setLayoutManager(new LinearLayoutManager(DoctorChooseSlots.this));
        ...
        start_view.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (chosen_date.equals("")) {
                    Toast.makeText(DoctorChooseSlots.this, "Select Date First", Toast.LENGTH_SHORT).show();
        ...
        choose.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (chosen_date.equals("")) {
                    Toast.makeText(DoctorChooseSlots.this, "Select Date", Toast.LENGTH_SHORT).show();
                    return;
                }
                if (start_data.equals("") || start_data.equals("Start Time")) {
                    Toast.makeText(DoctorChooseSlots.this, "Choose Start Time", Toast.LENGTH_SHORT).show();
                    return;
                }

                if (end_data.equals("") || end_data.equals("End Time")) {
                    Toast.makeText(DoctorChooseSlots.this, "Choose End Time", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                reference_booking.child(email).child(chosen_date).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if (snapshot.exists()) {
                            ...
                            if (flag == 1) {
                                Toast.makeText(DoctorChooseSlots.this, "You have already booked slot in this Time Frame", Toast.LENGTH_SHORT).show();
                                return;
                            } else {
                                int slots = (end - start) * 2;
                                String date = "";
                                DoctorTimeSlots time = new DoctorTimeSlots(String.valueOf(start), String.valueOf(end));
                                ...
                                for (int i = 0; i < slots; i++) {
                                    ...
                                    BookingAppointments booking = new BookingAppointments(0, "null");
                                    reference_booking.child(email).child(chosen_date).child(complete_slot).child(date).setValue(booking);
                                }
                                Toast.makeText(DoctorChooseSlots.this, "Slots Chosen", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            int slots = (end - start) * 2;
                            String date = "";
        ...
        reference_booking.child(email).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    adapter = new DoctorViewSlotsAdapter(DoctorChooseSlots.this, doctor_slots, date_slot, count);
                                            ...
                                            if (count != 0) {
                                                for (DataSnapshot datasnap : data.getChildren()) {
                                                    if (!(datasnap.getKey().equals("Count"))) {
                                                        ...
                                                        BookingAppointments appts = datasnap.getValue(BookingAppointments.class);
