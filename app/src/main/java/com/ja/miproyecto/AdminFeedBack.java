...
public class AdminFeedBack extends AppCompatActivity {
    ...
    private AdminFeedBackAdapter admin_feedBack_adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        reference = FirebaseDatabase.getInstance("https://mi-proyecto-8c7aa-default-rtdb.asia-southeast1.firebasedatabase.app/").getReference("Doctors_Feedback");
                ...
                reference.child(email).addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        if(snapshot.exists()) {
                            ...
                            admin_feedBack_adapter = new AdminFeedBackAdapter(email_val, name_val, dates, time, phone_num);
                            recyclerView.setAdapter(admin_feedBack_adapter);
                        }
                        else{
                            admin_feedBack_adapter = new AdminFeedBackAdapter(email_val, name_val, dates, time, phone_num);
                            recyclerView.setAdapter(admin_feedBack_adapter);
                            Toast.makeText(AdminFeedBack.this, "Doctor has no Current Feedbacks", Toast.LENGTH_SHORT).show();
