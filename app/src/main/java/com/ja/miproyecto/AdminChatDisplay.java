...
public class AdminChatDisplay extends AppCompatActivity {
    ...
    private void readChats(String email) {
        if(usersList.size() == 0){
            mUsers.clear();
            Toast.makeText(AdminChatDisplay.this, "Doctor has no Current Chats", Toast.LENGTH_SHORT).show();
            adminUserAdapter = new AdminUserAdapter(AdminChatDisplay.this, mUsers, email);
            recyclerView.setAdapter(adminUserAdapter);
        }
        else {
            reference = FirebaseDatabase.getInstance("https://mi-proyecto-8c7aa-default-rtdb.asia-southeast1.firebasedatabase.app").getReference("Patient_Details");
