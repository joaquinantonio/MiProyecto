...
public class PatientBookingAppointments extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        email = email.replace(".", ",");
        PatientSessionManagement session = new PatientSessionManagement(PatientBookingAppointments.this);
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (snapshot.exists()) {
                        ...
                        if (!(dataSnapshot.getKey().equals("Count"))) {
                            ...
                            for (int i = 0; i < slots; i++) {
                                ...
                                reference_booking.child(email).child(date_val).child(complete_slot).child(check).addValueEventListener(new ValueEventListener() {
                                    ...
                                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                                        if (snapshot.exists()) {
                                            BookingAppointments booking = snapshot.getValue(BookingAppointments.class);
                    ...
                    if(dates.size() == 0 && isIsresumed()) {
                        Toast.makeText(PatientBookingAppointments.this, "No Slots Available", Toast.LENGTH_SHORT).show();
                    }
                }
        ...
        book_app.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(chosen_time.equals("")){
                    Toast.makeText(PatientBookingAppointments.this, "Please Select the Time Slot", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                BookingAppointments booking = new BookingAppointments(1, phone);
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if(snapshot.exists()) {
                            ...
                            if (booking.getValue() == 1) {
                                Toast.makeText(PatientBookingAppointments.this, "The Slot is already booked. Please choose other slot", Toast.LENGTH_SHORT).show();
                                return;

                            } else {
                                BookingAppointments booking_appointments = new BookingAppointments(1, "null");
                                reference_booking.child(email).child(date_val).child(finalSlot_val).child(check).setValue(booking_appointments);
                                Intent intent = new Intent(PatientBookingAppointments.this, PatientPaymentAppointment.class);
                            ...
                            Toast.makeText(PatientBookingAppointments.this, "The Doctor is not available. Select other slot", Toast.LENGTH_SHORT).show();
                            return;
                        }
                    }
