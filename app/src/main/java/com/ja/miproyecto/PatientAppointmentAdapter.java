...
public class PatientAppointmentAdapter extends RecyclerView.Adapter<PatientAppointmentAdapter.ViewHolder>{
    ...
    private List<AdminPaymentClass> payments;

    public PatientAppointmentAdapter(List<AdminPaymentClass> payments) {
    ...
    public void onBindViewHolder(@NonNull PatientAppointmentAdapter.ViewHolder holder, int position) {
        AdminPaymentClass admin_payment_class = payments.get(position);
        holder.name.setText("Booked for: "+ admin_payment_class.getDname());
        holder.transaction.setText("Reference ID: "+ admin_payment_class.getTransaction());
        ...
        if(admin_payment_class.getPayment() == 0){
            holder.payment.setText("Payment in progress");
        }
        else if(admin_payment_class.getPayment() == 1){
            holder.payment.setText("Payment Confirmed");
            holder.payment.setTextColor(Color.GREEN);
        }
        else{
            holder.payment.setText("Invalid Reference ID. Payment Declined");
        }
        ...
        if(admin_payment_class.getPayment()==1 && admin_payment_class.getStatus()==0) {
            holder.itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    i.putExtra("title", "Video Appointment");
                    holder.itemView.getContext().startActivity(i);

                }
            });
        }
        DatabaseReference reference = FirebaseDatabase.getInstance("https://mi-proyecto-8c7aa-default-rtdb.asia-southeast1.firebasedatabase.app/").getReference("Doctors_Data");
